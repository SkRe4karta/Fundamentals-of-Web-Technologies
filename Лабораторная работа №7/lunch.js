...
// ЛР №7: сначала пытаемся получить меню с API.
// Если учебный сервер временно недоступен (например, 502),
// используем локальную копию данных, чтобы сайт оставался рабочим.
async function loadDishes() {
    try {
        ...
        console.warn("Учебный API недоступен. Используется локальная резервная копия меню.", error);
