...
function buildApiUrl(baseUrl, path) {
    ...
    // По условию ЛР №8–9 api_key всегда передаётся в строке запроса.
...
async function loadOrdersFromApi() {
    ...
    if (!Array.isArray(data)) {
        throw new Error("Сервер вернул список заказов в неожиданном формате");
    }
    return data;
}
...
async function updateOrderOnServer(orderId, payload) {
    ...
    return requestApi(`/orders/${Number(orderId)}`, {
        method: "PUT",
        headers: {
...
async function deleteOrderOnServer(orderId) {
    ...
    return requestApi(`/orders/${Number(orderId)}`, { method: "DELETE" });
}
